                ...
                for (a = 0; a < reply.length; a++) {
                    ...
                    if (lof !== null) {
                        ...
                        if (status === "NAR") {
                            ...
                            status = "Artwork Not Requested Confirmed";
                            col2.innerHTML = status;
                            col2.style.color = "#00008F";
...
<div id="page-wrapper">
    ...
    <div class="container-fluid">
        <div class="col-lg-10 col-lg-offset-1" style="padding-top: 3%;">
            <div class="row">
                <div class="panel-heading pull-right"><strong>Company</strong></div>
                <div class="panel-heading pull-right">
                    <select class="form-control" name="searchcompany" id="cmb_company" style="width: 100%;">
                        <sql:query var="idQcompany"  dataSource="jdbc/sdb">
                            SELECT * FROM `sys_user` JOIN `customer` ON `customer`.`username`= `sys_user`.`username` join company on company.company_id=sys_user.company_id  WHERE company.cs = '<%=user3.getUsername()%>'  GROUP BY (`company`.`company_id`);
                        </sql:query>
                        <option value="0" selected="selected" disabled="disabled">Sort By Company</option>
                        <c:forEach var="rowcompany" items="${idQcompany.rows}"> 
                            <option value="${rowcompany.company_id}">${rowcompany.company_name}</option>
                        </c:forEach>
                    </select>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $("#cmb_company").bind('input', function () {
            var temp = document.getElementById("cmb_company").value;
            
            document.getElementById('cmb_poNo').options.length = 1;
            document.getElementById('cmb_styNo').options.length = 1;
            document.getElementById('cmb_webNo').options.length = 1;
            document.getElementById('cmb_cusNo').options.length = 1;
            var data = temp + "@@@purchaseorder";
            console.log(temp + " && " + data);

            $.post('RequestAwController', {
                data: data,
                actionType: 'loadSelectBox'
            }, function (reply1) {
                var reply = JSON.parse(reply1);
                console.log(reply);

                if (reply.purchase_order_uk.length > 0) {

                    for (a = 0; a < reply.purchase_order_uk.length; a++) {

                        var lof = reply.purchase_order_uk[a];

                        if (lof !== null) {
                            var purchase_order_uk = lof.purchase_order_uk;
                            var selcmb_poNo = document.getElementById('cmb_poNo');
                            var optcmb_poNo = document.createElement('option');
                            optcmb_poNo.appendChild(document.createTextNode(purchase_order_uk));
                            optcmb_poNo.value = purchase_order_uk; // set value property of opt
                            selcmb_poNo.appendChild(optcmb_poNo); // add opt to end of select box (sel)                            
                        }
                    }
                }
                if (reply.id.length > 0) {

                    for (a = 0; a < reply.id.length; a++) {

                        var lof = reply.id[a];

                        if (lof !== null) {
                            var id = lof.id;
                            var selcmb_webNo = document.getElementById("cmb_webNo");
                            var optcmb_webNo = document.createElement("option");
                            optcmb_webNo.appendChild(document.createTextNode(id));
                            optcmb_webNo.value = id; // set value property of opt
                            selcmb_webNo.appendChild(optcmb_webNo); // add opt to end of select box (sel)
                        }
                    }
                }
                if (reply.style_no_uk.length > 0) {

                    for (a = 0; a < reply.style_no_uk.length; a++) {

                        var lof = reply.style_no_uk[a];

                        if (lof !== null) {

                            var style_no_uk = lof.style_no_uk;
                            var selcmb_styNo = document.getElementById('cmb_styNo');
                            var optcmb_styNo = document.createElement('option');
                            optcmb_styNo.appendChild(document.createTextNode(style_no_uk));
                            optcmb_styNo.value = style_no_uk; // set value property of opt
                            selcmb_styNo.appendChild(optcmb_styNo); // add opt to end of select box (sel)

                        }
                    }
                }
                if (reply.customer_po.length > 0) {

                    for (a = 0; a < reply.customer_po.length; a++) {

                        var lof = reply.customer_po[a];

                        if (lof !== null) {

                            var customer_po = lof.customer_po;
                            var selcmb_cusNo = document.getElementById('cmb_cusNo');
                            var optcmb_cusNo = document.createElement('option');
                            optcmb_cusNo.appendChild(document.createTextNode(customer_po));
                            optcmb_cusNo.value = customer_po; // set value property of opt
                            selcmb_cusNo.appendChild(optcmb_cusNo); // add opt to end of select box (sel)
                        }
                    }
                }
            });
